...
const CAT_ICONS = { grammar:"G",vocab:"V",listening:"L",reading:"R",speaking:"S",mock_ielts:"M" };
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(240px,1fr))", gap: 12 }}>
        {quizzes.map(q => (
          <div key={q.id} style={{ ...card, cursor: "pointer" }}
            ...
            <div style={{ fontSize: 20, fontWeight: 800, marginBottom: 8, color: "var(--accent)" }}>{CAT_ICONS[q.category] || "Q"}</div>
      ...
      {result && (
        <div style={{ marginTop: 12 }}>
          <div style={{ fontWeight: 700 }}>
            Band Estimate: {result.band_estimate ?? 'Unavailable'}
          </div>
  ...
  if (finished && result) return (
    <div style={card}>
      <div style={{ textAlign: "center", marginBottom: 20 }}>
        <div style={{ fontSize: 18, fontWeight: 800, marginBottom: 8, color: "var(--gold)" }}>{result.score >= 70 ? "PASSED" : "KEEP GOING"}</div>
...
/* ═══════════════════════════════════════════════════════
   4.  RESOURCES PAGE
═══════════════════════════════════════════════════════ */
const TYPE_ICONS = { link: "LINK", pdf: "PDF", video: "PLAY", audio: "AUDIO" };
        ...
        {resources.map(r => (
          <a key={r.id} href={r.url} target="_blank" rel="noopener noreferrer"
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
              <span style={{ fontSize: 11, fontWeight: 800, color: "var(--accent)" }}>{TYPE_ICONS[r.type] || "LINK"}</span>
