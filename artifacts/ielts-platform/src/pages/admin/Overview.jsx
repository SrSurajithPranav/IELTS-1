...
export default function AdminOverview() {
  ...
  const stats = [
    ...
    { label: 'Pending Reviews', value: pending.length, icon: 'R', color: 'var(--warn)' },
    { label: 'Active Today', value: students.filter((s) => s.last_active_date === new Date().toISOString().split('T')[0]).length, icon: 'A', color: 'var(--success)' },
    { label: 'Avg Score', value: students.length ? (students.reduce((a, s) => a + (s.score || 0), 0) / students.length).toFixed(1) : '—', icon: 'A', color: 'var(--gold)' },
  ];
