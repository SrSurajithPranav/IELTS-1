...
export default function GamesPage() {
  ...
  const [trainingBankReady, setTrainingBankReady] = useState(false);

  useEffect(() => {
    loadTrainingBank()
      .then(() => setTrainingBankReady(true))
      .catch(() => setTrainingBankReady(true));
  }, []);

  const startQuiz = (catId, config = quizConfig) => {
    if (!trainingBankReady) return;
    let questions;
    if (catId === 'mixed') {
  ...
  return (
    <div style={S.page}>
      ...
      {!trainingBankReady && (
        <div style={{ ...S.sub, color: 'var(--accent)' }}>Loading the full practice bank…</div>
      )}
      ...
      {selectedCategory && (
        <div className="fade-up" style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <Button onClick={() => startQuiz(selectedCategory)} disabled={!trainingBankReady} style={{ flex: 1 }}>
