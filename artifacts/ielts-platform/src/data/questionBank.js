...
// Keep the large training bank out of the JavaScript bundle. It is served as
// a static asset and Games loads it when that feature opens.
...
export const CATEGORIES = [
  { id: 'grammar', label: 'Grammar', icon: 'G', color: '#146c72', count: GRAMMAR_QUESTIONS.length },
  { id: 'vocabulary', label: 'Vocabulary', icon: 'V', color: '#6b46c1', count: VOCAB_QUESTIONS.length },
  { id: 'reading', label: 'Reading', icon: 'R', color: '#b7791f', count: READING_QUESTIONS.length },
  { id: 'listening', label: 'Listening', icon: 'L', color: '#2f855a', count: LISTENING_QUESTIONS.length },
  { id: 'writing', label: 'Writing', icon: 'W', color: '#2b6cb0', count: WRITING_QUESTIONS.length },
  { id: 'speaking', label: 'Speaking', icon: 'S', color: '#c53030', count: SPEAKING_QUESTIONS.length },
  { id: 'test_knowledge', label: 'Test Knowledge', icon: 'T', color: '#d69429', count: TEST_KNOWLEDGE_QUESTIONS.length },
...
let trainingBankPromise;
export function loadTrainingBank() {
  if (!trainingBankPromise) {
    trainingBankPromise = fetch('/ielts_massive_training_bank.json')
      .then((response) => {
        if (!response.ok) throw new Error('Training bank unavailable');
        return response.json();
      })
      .then((trainingBank) => {
        const normalized = normalizeTrainingBankData(trainingBank);
        const sections = {
          reading: READING_QUESTIONS,
          listening: LISTENING_QUESTIONS,
          writing: WRITING_QUESTIONS,
        ...
        Object.entries(sections).forEach(([skill, target]) => {
          target.push(...(normalized[skill] || []).map(toLegacyQuestion));
          ALL_QUESTIONS.push(...target);
          const category = CATEGORIES.find((item) => item.id === skill);
          if (category) category.count = target.length;
        });
        TOTAL_QUESTIONS = ALL_QUESTIONS.length;
        return TOTAL_QUESTIONS;
      });
  }
  return trainingBankPromise;
}
