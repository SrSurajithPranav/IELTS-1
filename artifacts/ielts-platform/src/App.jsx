...
const GlobalStyles = () => (
  <style>{`
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    html, body, #root { min-height: 100%; font-family: 'Manrope', sans-serif; background: var(--bg); color: var(--text); }
    body { background-image: radial-gradient(circle at 8% 0%, rgba(18,107,104,.045), transparent 32rem); }

    ::-webkit-scrollbar { width: 5px; }
    ::-webkit-scrollbar-track { background: var(--bg2); }
    ::-webkit-scrollbar-thumb { background: var(--border); border-radius: 4px; }

    .playfair { font-family: 'Fraunces', serif; letter-spacing: -.02em; }
...
const Sidebar = ({ page, setPage, user, onLogout }) => {
  const studentNav = [
    ...
    { id: "vocabulary",   icon: "V", label: "Vocabulary" },
    { id: "plans",        icon: "P", label: "Plans" },
    { id: "tasks",        icon: "✓",  label: "Today's Tasks" },
    { id: "speaking",     icon: "S", label: "Speaking" },
    { id: "writing",      icon: "W", label: "Writing" },
    { id: "debate",       icon: "D", label: "Debate Mode" },
    { id: "progress",     icon: "↗", label: "Progress" },
    { id: "ai-coach",     icon: "A", label: "AI Coach" },
    { id: "mocktest",     icon: "T",  label: "Mock Test" },
    { id: "games",        icon: "G", label: "Games" },
    { id: "leaderboard",  icon: "L", label: "Leaderboard" },
    { id: "liveclass",    icon: "C", label: "Live Class" },
    { id: "quizzes",      icon: "Q", label: "Quizzes" },
    { id: "resources",    icon: "R", label: "Resources" },
  ];
  const adminNav = [
    { id: "admin-home",     icon: "◈",  label: "Overview" },
    { id: "admin-students", icon: "S", label: "Students" },
    { id: "admin-plans",    icon: "P", label: "Plans" },
    { id: "admin-tasks",    icon: "✓",  label: "Tasks" },
    { id: "admin-review",    icon: "R", label: "Review" },
    { id: "admin-sessions",    icon: "C", label: "Sessions" },
    { id: "admin-resources",   icon: "L", label: "Resources" },
    { id: "admin-quizzes",     icon: "Q", label: "Quiz Builder" },
  ];
      ...
      <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border)" }}>
        ...
        {user?.role === "student" && user.streak > 0 && (
          <div style={{ marginTop: 10, display: "flex", alignItems: "center", gap: 6, background: "rgba(245,200,66,.1)", borderRadius: 8, padding: "6px 10px" }}>
             ...
             <span style={{ fontSize: 12, color: "var(--gold)", fontWeight: 600 }}>{user.streak} days</span>
          </div>
        )}
      </div>
...
const TaskCard = ({ task, onSubmit }) => {
  ...
  const iconMap = { speaking: "S", writing: "W", listening: "L", reading: "R", grammar: "G" };
  const colorMap = {
      ...
      <div style={{ padding: "18px 20px", display: "flex", alignItems: "center", gap: 14, cursor: "pointer" }}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {!submitted && <span style={{ color: "var(--muted)", fontSize: 12 }}>{expanded ? "▲" : "▼"}</span>}
           {submitted && task.status === "reviewed" && <span style={{ fontSize: 12, color: "var(--success)", fontWeight: 700 }}>REVIEWED</span>}
        </div>
      </div>
...
const StudentDashboard = ({ user }) => {
  ...
  const stats = [
    { label: "Day",      value: `Day ${todayTasks[0]?.day_number || 0}`, icon: "D", color: "var(--accent)" },
    { label: "Score",    value: user.score,      icon: "S", color: "var(--gold)" },
    { label: "Streak",   value: `${user.streak} days`, icon: "", color: "var(--warn)" },
    { label: "Reviewed", value: `${todaySubmissions.filter((s) => s.status === "reviewed").length} tasks`, icon: "R", color: "var(--success)" },
  ];
...
const SpeakingFollowupPanel = () => {
  ...
  return (
    <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: 18, marginBottom: 20 }}>
      <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 12 }}>AI Follow-up Questions</div>
    ...
    <div>
      ...
      <Card className="fade-up-3" style={{ border: "2px dashed var(--border)" }}>
        <div style={{ textAlign: "center", padding: "20px 0" }}>
          <div style={{ fontSize: 24, marginBottom: 10, fontWeight: 700, color: "var(--accent)" }}>S</div>
    ...
    <div>
      ...
      <Card className="fade-up-5" style={{ background: "rgba(245,200,66,.06)", border: "1px solid rgba(245,200,66,.2)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <div style={{ fontSize: 18, fontWeight: 800, color: "var(--gold)" }}>STREAK</div>
          <div>
      ...
      <div className="fade-up-2">
        {leaderboardData.length === 0 && (
          <Card style={{ textAlign: "center", padding: 40 }}>
            <div style={{ fontSize: 18, fontWeight: 800, marginBottom: 12, color: "var(--gold)" }}>LEADERBOARD</div>
    ...
    <div>
      ...
      <Card className="fade-up-2" style={{ marginBottom: 20, background: "linear-gradient(135deg,rgba(20,108,114,.12),rgba(214,148,41,.08))", border: "1px solid rgba(20,108,114,.2)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 18 }}>
          <div style={{ width: 48, height: 48, borderRadius: 12, background: "var(--accent)", color: "var(--on-accent)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 800 }}>LIVE</div>
          <div>
...
const AdminHome = () => {
  ...
  const stats = [
    { label: "Total Students", value: students.length, icon: "S", color: "var(--accent)" },
    { label: "Active Plans", value: plans.length, icon: "P", color: "var(--success)" },
    { label: "Pending Review", value: pending.length, icon: "R", color: "var(--warn)" },
    { label: "Avg Score", value: avgScore, icon: "A", color: "var(--gold)" },
  ];
  return (
    <div>
    ...
    <div>
      ...
      {!loadingStudents && students.length === 0 && (
        <Card style={{ marginBottom: 16, textAlign: "center", padding: 28 }}>
          <div style={{ fontSize: 18, fontWeight: 800, marginBottom: 10, color: "var(--accent)" }}>STUDENTS</div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                ...
                <div style={{ textAlign: "right" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4, justifyContent: "flex-end" }}>
                    <span style={{ fontSize: 12 }}>{s.streak} streak</span>
          ...
          <Card>
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
              {[
                ...
                { l: "Streak", v: `${selected.streak} days`, c: "var(--warn)" },
...
export default function App() {
  ...
  const [authChecking, setAuthChecking] = useState(() => Boolean(localStorage.getItem("jwt_token")));
  ...
  if (authChecking) return (
    <>
      <GlobalStyles />
      <div className="auth-boot" aria-label="Loading your workspace">
        <div className="auth-boot-mark">IP</div>
        <div className="skeleton auth-boot-line" />
        <div className="skeleton auth-boot-line short" />
      </div>
    </>
  );
    ...
    <ThemeProvider>
      <NotificationProvider>
        ...
        <div style={{ display: "flex", minHeight: "100vh" }}>
          ...
          <main className="legacy-shell-main" style={{ marginLeft: "var(--sidebar-w)", flex: 1, padding: "32px 32px 32px", minHeight: "100vh", overflowY: "auto" }}>
            <div className="legacy-shell-content" style={{ maxWidth: 860 }}>
              {content}
            </div>
          </main>
        </div>
        <nav className="mobile-nav" aria-label="Primary navigation">
          {[
            ...
            <button
              key={id}
              type="button"
              data-testid={`button-mobile-${id}`}
              onClick={() => setPage(id)}
              className={page === id ? "active" : ""}
            >
              <span>{id === "dashboard" ? "◈" : id === "progress" ? "↗" : id.slice(0, 1).toUpperCase()}</span>
              {label}
            </button>
          ))}
        </nav>
